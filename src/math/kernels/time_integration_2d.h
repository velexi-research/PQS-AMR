...
#ifndef INCLUDED_PQS_math_time_integration_2d_h
#define INCLUDED_PQS_math_time_integration_2d_h
...
void PQS_MATH_2D_RK1_STEP(
  PQS_REAL *u_next,
  const int *u_next_gb_lo,
  const int *u_next_gb_hi,
  const PQS_REAL *u_current,
  const int *u_current_gb_lo,
  const int *u_current_gb_hi,
  const PQS_REAL *rhs,
  const int *rhs_gb_lo,
  const int *rhs_gb_hi,
...
void PQS_MATH_2D_TVD_RK2_STAGE1(
  PQS_REAL *u_stage1,
  const int *u_stage1_gb_lo,
  const int *u_stage1_gb_hi,
  const PQS_REAL *u_current,
  const int *u_current_gb_lo,
  const int *u_current_gb_hi,
  const PQS_REAL *rhs,
...
void PQS_MATH_2D_TVD_RK3_STAGE2(
  PQS_REAL *u_stage2,
  const int *u_stage2_gb_lo,
  const int *u_stage2_gb_hi,
  PQS_REAL *u_stage1,
  const int *u_stage1_gb_lo,
  const int *u_stage1_gb_hi,
  const PQS_REAL *u_current,
