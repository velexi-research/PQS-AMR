 ...
 * use in example application that has no solid phase.
 */
    ...
    if (d_dim == 2) {
        if (d_interface_type == "circle") {
            INIT_CIRCLE(phi,
                        ...
                        d_center,
                        &d_radius);
        } else if (d_interface_type == "starfish") {
            INIT_STARFISH(phi,
                          phi_ghostbox_lo, phi_ghostbox_hi,
                          fillbox_lo, fillbox_hi,
                          x_lower,
            ...
            PQS_ERROR(this, "initializeInterface",
                      string("interface_type=") + d_interface_type +
                      string(". Valid values: circle, starfish"));
        }
    } else if (d_dim == 3) {
        if (d_interface_type == "sphere") {
            INIT_SPHERE(phi,
                        phi_ghostbox_lo, phi_ghostbox_hi,
                        fillbox_lo, fillbox_hi,
                        x_lower,
                        ...
                        d_center,
                        &d_radius);
        } else if (d_interface_type == "bumpy sphere") {
            INIT_BUMPY_SPHERE(phi,
                          phi_ghostbox_lo, phi_ghostbox_hi,
                          fillbox_lo, fillbox_hi,
                          x_lower,
                          dx,
    ...
    const shared_ptr<tbox::Database>& config_db) const
{
    ...
    if (!config_db->isString("interface_type")) {
        PQS_ERROR(this, "verifyConfigurationDatabase",
                  "'interface_type' missing from configuration database");
    }

    if (!config_db->keyExists("center")) {
        PQS_ERROR(this, "verifyConfigurationDatabase",
                  "'center' missing from configuration database");
    }
    if (!config_db->isDouble("radius")) {
        PQS_ERROR(this, "verifyConfigurationDatabase",
                  "'radius' missing from 'PQS' database");
    }
        ...
        const shared_ptr<tbox::Database>& config_db)
{
    ...
    d_interface_type = config_db->getString("interface_type");
    config_db->getDoubleArray("center", d_center, d_dim);
